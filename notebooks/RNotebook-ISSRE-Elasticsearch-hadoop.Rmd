...
###Dataset:
Aqui é apresentado o raw ("cru") dataset que foi discutido na ultima reunião conforme planilha que o Biel gerou no Drive. Tal dataset consiste na relação entre os valores de mudanças e inserção de bugs commit. Segue uma descrição das respectivas colunas:
...
Obs: Ressaltando que os dados estão desnormalizados, portanto há uma repetição de alguns valores em algumas colunas, ex: NoTimeSeries, na linha 2 e 3 existem 1681 de maneira repetida, mas no cálculo é utilizado somente um valor, visto que existem somente 1681 time series com 2 commits.
...
Obs: Como os dados anteriores tem algumas colunas desnormalizadas (NoTimeSeries), algumas formulas contém o valor max do respectiva coluna dentro do agrupamento definido.
